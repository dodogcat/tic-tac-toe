프로그램 개요
게임 시작 후 심판은 다음 플레이어를 정한다.
해당 플레이어는 자신의 차례에 말을 둔다.
심판은 게임이 끝나는지 확인한다.
게임이 끝나지 않으면 다음 플레이어를 정한다.
...
후기
기존의 코드를 SOLID 원칙에 맞게 바꾸는 과정에서 많은 부분을 새로 만들다보니
기존 코드에 비해 가독성이 떨어지는 기분이 들었다.
그래도 심판의 역할과 플레이어들의 역할에 대한 설명을 들으면 직관적이고
유지보수가 쉽게 사용 가능할 것이라고 생각한다.
